    ...
    <div class="justify-center">
        <div class="w-96 m-auto h-28" >
            ...
            <div v-if="drawEnded" class="w-full flex justify-between">
                ...
                <div v-if="lastScore >= 70" class="mt-9 h-7 text-slate-100 text-lg w-28 font-bold  rounded-full py-0 px-4 ">
                    Správne
...
export default {
    ...
    methods: {
        ...
        getShortestDistance(correctPoints, userX, userY) {
            let shortestDistance = Infinity; // Start with the largest possible distance

            for (let point of correctPoints) {
                // Calculate Euclidean distance
                let distance = Math.sqrt(Math.pow(point.x - userX, 2) + Math.pow(point.y - userY, 2));

                // If the calculated distance is less than the shortest known distance, update closest point and shortest distance
        ...
        endDrawing(context, width, height, points) {
            ...
            for (let x = -10; x <= 10; x += 0.05) {
                ...
                const userX = x;
                let matchingPoints = userInterpolatedPoints.filter(point => point.x == x);
                let userY = matchingPoints.length > 0 ? matchingPoints[0].y : null;

                if (userY != null) {
                    let shortestDistance = this.getShortestDistance(correctPoints, userX, userY);
                    console.log("userX: " + userX + " userY: " + userY)
                    console.log("shortestDistance: " + shortestDistance)

                    if (shortestDistance < this.data.maxDeviation) {
